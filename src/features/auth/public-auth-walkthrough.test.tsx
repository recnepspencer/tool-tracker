...
describe('public auth parity routes', () => {
  ...
  it('previews signup and company setup without calling an adapter or persisting the draft', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Preview workspace' }));
    expect(screen.getByRole('alert')).toHaveTextContent('Enter a company name to preview the setup.');
    ...
    await user.click(screen.getByRole('button', { name: 'Preview workspace' }));
    expect(await screen.findByRole('heading', { name: 'Your preview is ready' })).toBeInTheDocument();
  ...
  it('shows valid and unknown invitation states without activating a person', async () => {
    ...
    expect(await screen.findByRole('heading', { name: 'Join the demo workspace' })).toBeInTheDocument();
    await user.click(screen.getByRole('button', { name: 'Preview acceptance' }));
    expect(await screen.findByRole('heading', { name: 'Invitation preview accepted' })).toBeInTheDocument();
    expect(Object.values(calls).every((count) => count === 0)).toBe(true);
    expect(storageKeys(localStorage)).toEqual(['nelson-demo-theme']);
    expect(storageKeys(sessionStorage)).toEqual([]);
    ...
    expect(screen.getByText('No demo invitation matches this link.')).toBeInTheDocument();
  ...
  it('walks through recovery locally and never writes a password', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Continue' }));
    const codeHeading = await screen.findByRole('heading', { name: 'Enter the demo code' });
    expect(document.activeElement).toBe(codeHeading);
    const code = screen.getByLabelText('Demo code');
    await user.click(screen.getByRole('button', { name: 'Verify code' }));
    expect(screen.getByRole('alert')).toHaveTextContent('Enter the six-digit demo code 123456.');
    ...
    await user.click(screen.getByRole('button', { name: 'Verify code' }));
    const passwordHeading = await screen.findByRole('heading', { name: 'Choose a demo password' });
    expect(document.activeElement).toBe(passwordHeading);
    const password = screen.getByLabelText('New demo password');
    await user.click(screen.getByRole('button', { name: 'Finish preview' }));
    expect(screen.getByRole('alert')).toHaveTextContent('Use at least eight characters for the demo password.');
    ...
    await user.click(screen.getByRole('button', { name: 'Finish preview' }));
    expect(await screen.findByRole('heading', { name: 'Recovery preview finished' })).toBeInTheDocument();
