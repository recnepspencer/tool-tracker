...
- Ray's worker routes (`tools`, `activity`, `checkout`, and `account`) and Sam's admin routes (`dashboard`, `activity`, `settings`, and warehouse operations) were opened at 320, 390, and 768 CSS pixels.
...
- Reset demo data was confirmed with the exact `RESET DEMO DATA` phrase, restoring the seed.
